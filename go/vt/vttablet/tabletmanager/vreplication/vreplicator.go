...
func (vr *vreplicator) buildColInfoMap(ctx context.Context) (map[string][]*ColumnInfo, error) {
	...
	for _, td := range schema.TableDefinitions {
		query := fmt.Sprintf(queryTemplate, encodeString(schemaName), encodeString(td.Name))
		...
		if len(td.PrimaryKeyColumns) != 0 {
			...
			if pks, _, err = mysqlctl.GetPrimaryKeyEquivalentColumns(ctx, executeFetch, schemaName, td.Name); err != nil {
				return nil, err
			}
	...
	if len(secondaryKeys) > 0 {
		alterDrop := &sqlparser.AlterTable{
			Table: sqlparser.TableName{
				Qualifier: sqlparser.NewIdentifierCS(vr.getTargetSchemaName()),
...
// getTargetSchemaName returns the appropriate schema name for virtual keyspace support
func (vr *vreplicator) getTargetSchemaName() string {
	if vr.targetSchema != "" {
		return vr.targetSchema
	}
