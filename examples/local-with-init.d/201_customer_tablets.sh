...
# or you can copy vitess.yaml.201-example to /etc/vitess.yaml.
# We will do the latter here, but if you diff the 101 and 201 files,
# you can see all we are doing is adding 3 tablets.
cp ../common/config/vitess.yaml.201-example /etc/vitess.yaml
...
vtctldclient SetKeyspaceDurabilityPolicy --durability-policy=semi_sync customer || fail "Failed to set keyspace durability policy on the customer keyspace"
